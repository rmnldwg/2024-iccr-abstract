...
![Parametrized graph representation of the lymphatic network considering four LNLs. Blue, round nodes represent the hidden RVs, while orange square nodes show the observed RVs. Arcs represent a conditional probability parametrized with the quantity noted next to it.](../static/graph_with_obs.svg){#fig-graph-with-obs width="22%"}
...
To train our model, we need to compute the likelihood of a dataset $\mathbf{D}=\left( \mathbf{y}_1, \mathbf{y}_2, \ldots, \mathbf{y}_N \right)$ of $N$ patients, given the model's parameters $\boldsymbol{\theta}$. This is given by a product of the sums in the denominator of @eq-bayes-law:

$$
P \left(\mathbf{D} \mid \boldsymbol{\theta} \right) = \prod_{i=1}^N \underbrace{\sum_{j=1}^{2^V} P \left( \mathbf{Y} = \mathbf{y}_i \mid \mathbf{X} = \boldsymbol{\xi}_j \right) P \left( \mathbf{X} = \boldsymbol{\xi}_j \mid T_i, \boldsymbol{\theta} \right)}_{P \left( \mathbf{Y} = \mathbf{y}_i \mid T_i, \boldsymbol{\theta} \right)}
$$ {#eq-likelihood}
...
Introducing this latent variable is helpful, because the mixture model likelihood is invariant w.r.t. permutations of the component labels. This introduces a number of problems, e.g. for common MCMC sampling methods. The latent variable $\mathbf{Z}$ allows us to derive two sets of interdependent equations that we may solve in an iterative fashion (see e.g. @bishop_pattern_2006 for a detailed derivation) that is commonly referred to as _expectation-maximization (EM)_ algorithm:

The first are the probabilities of subcohort $i$ to belong to component $j$, given a set of parameters $\boldsymbol{\Psi}^\star$. These are often called the _responsibilities_:
$$
...
From this, we can compute new mixing proportions $c_j^\star=\sum_{i=1}^s \gamma (z_{ij}) / s$ and then infer new parameters $\boldsymbol{\Psi}^\star$ -- e.g. via MCMC sampling -- from the resulting likelihood:
