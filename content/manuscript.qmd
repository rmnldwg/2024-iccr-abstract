...
where $T \in \{ \text{early}, \text{advanced} \}$ denotes the T-category. We fix $P_\text{early}(t)$ to a binomial distribution with parameters $n=t_\text{max}=$ {{< var model.max_t >}} and $p_\text{early}=$ {{< var model.first_binom_prob >}}, while the advanced T-category's time-prior is also a binomial distribution where the $p_\text{advanced}$ parameter is learned.
...
Here, the $\mathbf{c} \in [0,1]^g$ is the vector of mixing proportions with $\sum_{j=0}^g c_j = 1$, while the $\boldsymbol{\Psi} = \left( \boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_g\right)$ is the vector of all $g$ models' parameters.
