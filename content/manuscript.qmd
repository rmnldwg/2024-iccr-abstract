...
![Parametrized graph representation of the lymphatic network considering four LNLs. Blue, round nodes represent the hidden RVs, while orange square nodes show the observed RVs. Arcs represent a conditional probability parametrized with the quantity noted next to it.](../static/graph_with_obs.svg){#fig-graph-with-obs width="20%"}
...
When a patient gets diagnosed, their LNLs' involvement status is assessed using a clinical modality (e.g. computed tomography (CT) or magnetic resonance imaging (MRI)) with its repsective sensitivity $s_N$ and specificity $s_P$. This involvement status, too, is modelled as an RV $Y_v$. Hence, a diagnosis obtained with this diagnostic modality can be collected in the random vector $\mathbf{Y}$.

We model the process of tumor progression via lymphatic drainage by connecting the hidden RVs in a graph, as shown in @fig-graph-with-obs. The arcs in [this graph](#fig-graph-with-obs) represent conditional probabilities. The orange arcs, as mentioned, correspond to observing a diagnosis $Y_v$, given a hidden state $X_v$. The values this conditional probability $P(Y_v \mid X_v)$ can take on are tabulated in @tbl-sens-spec.
